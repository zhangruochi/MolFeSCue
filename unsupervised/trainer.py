...
from unsupervised.utils.utils import is_parallel
...
class Masking_Trainer(object):
    def __init__(self, 
        ...
        self.model, self.linear_pred_atoms, self.linear_pred_bonds = model_list
        self.optimizer_model, self.optimizer_linear_pred_atoms, self.optimizer_linear_pred_bonds = optimizer_list
        ...
        for step, batch in enumerate(self.dataloaders['train']):
            ...
            if (self.global_train_step+1) % self.cfg.logger.log_per_steps == 0:
                ...
                if not self.cfg.mode.nni and self.global_rank == 0 and self.cfg.logger.log:
                    ...
                    mlflow.log_metric("train/acc_node", acc_node_accum/(step+1), step=self.global_train_step)
